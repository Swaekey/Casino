...
public class CardDeckAPITranslator {
    ...
    public static void newDeck(int _numDecks) {
        String callAction = "/new/shuffle/?deck_count=" + _numDecks;
        String urlString = BASE_URL + callAction;
        URL url;
        try {
            // Make the connection.
    ...
    public static CardsAPITranslator drawCardFromDeck() {
        String callAction = "/" + deckID + "/draw/?count=1";
        String urlString = BASE_URL + callAction;
        URL url;
        CardsAPITranslator drawnCard = new CardsAPITranslator("", "", 0);
        try {
            // Make the connection.
            url = new URL(urlString);
            HttpURLConnection con = (HttpURLConnection) url.openConnection();
    ...
    public static void shufflePile(String _pileName) {
        String callAction = "/" + deckID + "/pile/" + _pileName + "/shuffle/";
        String urlString = BASE_URL + callAction;
        URL url;
        try {
            // Make the connection.
    ...
    public static CardsAPITranslator addToPileFromDeck(String _pileName) {
        String callAction = "/" + deckID + "/pile/" + _pileName + "/add/?cards=";
        CardsAPITranslator cardInfo = drawCardFromDeck();
    ...
    public static int getPileRemaining(String _pileName) {
        String callAction = "/" + deckID + "/pile/" + _pileName + "/list";
        String urlString = BASE_URL + callAction;
        URL url;
        try {
            // Make the connection.
            ...
            if (status != 200) {
                ...
                // initialize variables
                JSONObject thisPile = piles.getJSONObject(_pileName);
    ...
    public static CardsAPITranslator drawCardFromPile(String _pileName) {
        String callAction = "/" + deckID + "/pile/" + _pileName + "/draw/?count=1";
        String urlString = BASE_URL + callAction;
        URL url;
        CardsAPITranslator drawnCard = new CardsAPITranslator("", "", 0);
        try {
    ...
    public static CardsAPITranslator addToPileFromPile(String _pileFrom, String _pileTo) {
        String callAction = "/" + deckID + "/pile/" + _pileTo + "/add/?cards=";
        CardsAPITranslator cardInfo = drawCardFromPile(_pileFrom);
        String card = cardInfo.getCardCode();
        String urlString = BASE_URL + callAction + card;
        URL url;
        try {
            ...
            if (status != 200) {
                ...
                //System.out.println(card + " is in " + _pileName);
                // Close the connections.
                in.close();
                con.disconnect();
